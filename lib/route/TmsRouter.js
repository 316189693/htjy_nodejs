...
class TmsRouter {
    constructor(config, logger, mysql, _, moment) {
        this.mysql = mysql;
        this.logger = logger;
        this.config = config;
        this._ = _;
        ...
        this.routers = [
...

            {
                ...
                'func': this.controller(this.validateCompanyIds, async function (req) {
                    ...
                    let rst = await p44Service.fetchShipmentCreatedOrders(req.query.companyIds, limit_num, order_id);
                    let num = 0;
                    if (rst) {
                        num = rst.length;
...

            {
                ...
                'func': this.controller(this.validateCompanyIds, async function (req) {
                    let limit_num =req.query.limit || 500;
                    let rst = await p44Service.fetchDispatchedOrders(req.query.companyIds, limit_num);
                    let num = 0;
                    if (rst) {
                        num = rst.length;
...

            {
                ...
                'func': this.controller(this.validateCompanyIds, async function (req) {
                    let limit_num =req.query.limit || 500;
                    let order_id = req.query.orderId;
                    let rst = await p44Service.fetchOrdersOfEnabledLoadP44TrackingStatusCarrierByCompanyIds(req.query.companyIds, limit_num, order_id);
                    let num = 0;
                    if (rst) {
                        num = rst.length;
...

            {
                ...
                'func': this.controller(this.validateCompanyIds, async function (req) {
                    let limit_num =req.query.limit || 500;
                    let rst = await p44Service.fetchOrdersOfEnabledPushTrackingStatusToP44CarrierByCompanyIds(req.query.companyIds, limit_num);
                    let num = 0;
                    if (rst) {
                        num = rst.length;
