...
class P44Service {
    ...
    async createLTLShipment(apiTransferLine) {
        let [p44Config, orderInfo] = await Promise.all([
            this.fetchP44ApiConfig(apiTransferLine['reference']),
            this.fetchOrderInfo(apiTransferLine['tms_order_id'])
        ]);
        let data = this.createLtlShipmentRequest(p44Config, orderInfo);
        let url = P44Apis.put_project_44_ltl_create_shipments_url;
        let transfer_line_id = apiTransferLine['tms_api_transfer_id'];
        let response = await axios_util.put(this.axiosInstance, url, data);
        let request_str = this.mysql.escapeStr(JSON.stringify({ "url": url, "method": "PUT", "request": data }));
    ...
    async trackedLTLShipment(apiTransferLine) {
        let [p44Config, orderInfo] = await Promise.all([
            this.fetchP44ApiConfig(apiTransferLine['reference']),
            this.fetchOrderInfo(apiTransferLine['tms_order_id'])
        ]);
        ...
        let request_str = this.mysql.escapeStr(JSON.stringify({ "url": url, "method": "POST", "request": data }));
        let response_str = "''";
    ...
    async fetchOrderInfo(tms_order_id) {
        let sql = `select tms_order_pro, 
    ...
    async fetchShipmentCreatedOrders(companyIds, limit_num, order_id) {
        ...
        let sql = `
                 ...
                 inner join (
                             select tms_order.tms_order_id, tms_order.tms_order_pro, tms_order.fk_company_id
                             from tms_order 
                             where  date(tms_order.tms_order_created_date) >  date('2020-07-15') 
    ...
    async fetchDispatchedOrders(companyIds, limit_num) {
        if (!this.commonUtil.isArray(companyIds)) {
            return null;
        }
        let companyIdStr = companyIds.join();
        ...
        let sql = `
                 ...
                 inner join (
                             select tms_order.tms_order_id, tms_order.tms_order_pro, tms_order.fk_company_id
                             from tms_order where
                                           date(tms_order.tms_order_created_date) >  date('2020-07-15')
                                            ${tms_order_company_filter}
                                           and  exists(select 1 
                                                  from tms_api_transfer 
                                                  where tms_api_transfer.fk_tms_order_id = tms_order.tms_order_id  
    ...
    /**
      ...
      * it is the column 'tms_p44_api_config_shipper_push_tracking_updates_to_project_44', 1 means enabled, 
      0 is disabled 
     */
    async fetchOrdersOfEnabledPushTrackingStatusToP44CarrierByCompanyIds(companyIds, limit_num) {
        if (!this.commonUtil.isArray(companyIds)) {
            return null;
        }
        let companyIdStr = companyIds.join();
    ...
    * it is the column 'tms_p44_api_config_carrier_load_tracking_updates_from_project_44', 1 means enabled, 
    0 is disabled 
   */
    async fetchOrdersOfEnabledLoadP44TrackingStatusCarrierByCompanyIds(companyIds,limit_num, order_id) {
        if (!this.commonUtil.isArray(companyIds)) {
            return null;
        }
        let order_id_filter = "";
        ...
        let sql = ` select tms_order.tms_order_id, tms_order.tms_order_pro, tms_order.fk_company_id, tms_p44_api_config.tms_p44_api_config_id
	              ...
	              inner join (
	               ...
	               limit ${limit_num};         
                   `;
        let rst = await this.mysql.query(sql);
        return rst;
    }
    ...
    async pushTmsOrdersStatus(tms_api_transfer_id, order_id, pro, tms_api_config_id) {
        ...
        let current_order_info = await this.fetchOrderInfo(order_id);
        ...
        if (status_code && (old_tms_order_info == null || old_tms_order_info['status_code'] != status_code)) {
            ...
            request_str = this.mysql.escapeStr(JSON.stringify({ "url": P44Apis.post_project_44_ltl_status_update_url, "method": "post", "request": request }));
            ...
            request_str = this.mysql.escapeStr(JSON.stringify({ "current_status": current_status, "current_stage": current_stage, "status_code": status_code, "old_status_code": old_tms_order_info['status_code'] }));
            tms_api_transfer_internal_status = " ,tms_api_transfer_internal_status='ERROR' ";
        }
     ...
     */
    async  updateTmsOrderStatus(tms_api_transfer_id, order_id, pro) {
        ...
        let status_reason = lastStatus.statusReason && lastStatus.statusReason.description ? (" Reason:" + lastStatus.statusReason.description) : "";
        let log_text = lastStatus.statusCode + status_reason;
        ...
        if (lastStatus) {
            ...
            if (parseInt(raw['num']) <= 0) {
                ...
                let tms_order = await this.mysql.assocQuery(`select fk_company_id,fk_tms_order_group_id,tms_order_stage from tms_order where tms_order_id = ${order_id_escape};`);
                ...
                await this.mysql.query(`
                                      ...
                                      VALUES(  
                                          ...
                                          ${log_datetime}, ${tms_order.fk_tms_order_group_id}, 0,
                                           ${tms_order.tms_order_stage}, 44
                                        );`
                );
                tms_api_transfer_status = ApiStatus.SUCCESS;
